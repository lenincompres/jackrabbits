...
class LightBox extends HTMLElement {
  ...
  constructor(content) {
    super();
    Binder.set(this, "opened");
    //this._opened.onChange(val => DOM.set(val ? 'hidden' : 'initial', 'overflow'));
    this.mainElement = DOM.element();
    if (content) this.add(content);
    this.set({
      class: {
        opened: this._opened.as(v => !!v),
      },
      zIndex: 1000,
      ...
      section: {
        ...
        div: this.mainElement.set(this.pager ? this.pager._content : content),
  ...
  add(content) {
    if (!content) return;
    if (this.pager && typeof content === "string") {
      if(this.pager.key === content) return;
      return this.pager.key = content;
    }
  ...
  open(content) {
    if (content) this.add(content);
    if (!document.body.contains(this)) DOM.set(this);
    this.opened = true;
  }
